...
* `THREAD`: Java线程数, active为当前线程数, daemon为当前线程中的daemon线程数, peak为历史最高线程数, new为新创建的线程数。
* `CLASS`: 新加载类时为黄色，没有红色。
...
* 进程CPU：服务器总CPU大于50%黄， 70%红
* 进程线程：如果服务器少于8核，线程数大于核数＊150 黄，核数＊225 红。 如果大于8核，核数＊100 黄, 核数＊150红。
* 进程内存： swap一旦使用即为红
* 进程磁盘IO: 读写磁盘大于50MB/s 黄，大于100MB/s 红
* JVM内存: 老生代，永久代，CodeCache，如果设置了Max，则大于85%黄，95%红
* JVM线程: active规则同进程线程，new规则：新建1条线程为黄，每秒创建一条为红。
* YGC: YGC次数每秒1次黄，大于2次红，平均耗时100ms黄，200ms红, 总耗时达到了应用运行时间的5%黄，10%红
* FGC: 次数：1次黄，大于2次红
* SAFEPOINT: 安全点次数每秒大于2次黄，4次红，总耗时达到了应用运行时间的5%黄，10%红
